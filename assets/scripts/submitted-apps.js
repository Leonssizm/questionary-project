...
fetch("https://bootcamp-2022.devtest.ge/api/applications?token=" + localStorage.getItem("token"))
  .then((response) => response.json())
  .then((data) => {
    data.forEach((form, index) => {
      let skillsData = "";

      workPreferenceData = addWorkPreference(form);
      covidData = addHadCovid(form);
      vaccinationData = addVaccinated(form);
      insightsData = addInsightsData(form);
...
function addWorkPreference(form) {
